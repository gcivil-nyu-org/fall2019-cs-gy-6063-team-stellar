...
def save_users(path,userlist):
    ...
    with open(path, 'w') as f:
        ...
        for user in userlist:
            ...
            cur.execute("INSERT INTO userrequest (user_id, service_type,time_stamp, cuisine, school, department) VALUES (?, ?, ?, ? , ? ,?)",
                        (user["user_id"], user["service_type"], datetime.datetime.now() , str(user["prefered cuisines"]), user["school"], user["prefered departments"]))
    conn.commit()
    conn.close()
