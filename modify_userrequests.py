...
def change_users(userlist):
    for user in userlist:
        r = UserRequest.objects.get(user=user["user"])
        r.service_type = (user["service_type"],)
        r.school = (user["school"][0].name,)
        r.department = (user["department"][0].name,)
        r.cuisines.clear()
        r.save()
